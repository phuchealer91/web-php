...
session_start();
	...
	if(isset($_SESSION['userName']) && isset($_SESSION['pass']) && isset($_POST['sync'])){
		if($_POST['sync'] == 1){
			...
			$query_soHoaDon = "select so_hoa_don from DatHang where ma_user =".$maNguoiDung." and trang_thai = 0";
			...
			if($checkBillUser > 0){
				...
				$query_tbSPHoaDon = "select * from ChiTietDonHang where so_hoa_don ='".$soHoaDon."'";
				...
				if($checkInfoBillUser > 0){
					...
					while($row = mysqli_fetch_array($tbSPHoaDon)){
						...
						if(isset($_COOKIE[$row['ma_san_pham']])){
							...
							$query_soLuong = 'select so_luong from ChiTietDonHang where ma_san_pham ="'.$row['ma_san_pham'].'" and so_hoa_don = "'.$soHoaDon.'"';
							$soLuongUpdate = mysqli_fetch_array($dbQuanLyBanLinhKien->query($query_soLuong));
							$soLuongud = $soLuongUpdate['so_luong']+1;
							$query_updateInfoBill = 'update ChiTietDonHang set so_luong = '.$soLuongud.' where ma_san_pham = "'.$row['ma_san_pham'].'" and so_hoa_don = "'.$soHoaDon.'"';
					...
					foreach($arrayMaSanPham as $maSanPham){
						if(in_array($maSanPham,$arrMaSPTonTai)){}
							...
							$query_dongia = 'select don_gia from HangHoa where ma_san_pham ='.$maSanPham;
							$arrdonGia = mysqli_fetch_array($dbQuanLyBanLinhKien->query($query_dongia));
							$donGia = $arrdonGia['don_gia'];
							$query_insertInfoBill = "INSERT INTO  ChiTietDonHang(so_hoa_don,ma_san_pham,so_luong, don_gia) VALUES('".$soHoaDon."', '".$maSanPham."',".$_COOKIE[$maSanPham].",".$donGia.")";
			...
			// kết thúc kiểm tra bill tồn tại hay không
				...
				$query_numBillUser = 'select * from DatHang';
				$numBillUser = mysqli_num_rows($dbQuanLyBanLinhKien->query($query_numBillUser))+1;
				$ngay_hd = date("Y-m-d H:i:s");
				$query_insertBillUser = "INSERT INTO `DatHang` (`so_hoa_don`, `ngay_hd`, `ma_user`, `tri_gia`, `trang_thai`) VALUES (".$numBillUser.", '".$ngay_hd."', ".$maNguoiDung.", 0,0)";
				$dbQuanLyBanLinhKien->query($query_insertBillUser);
				foreach($arrayMaSanPham as $maSanPham){
					$query_dongia = 'select don_gia from HangHoa where ma_san_pham ='.$maSanPham;
					$arrdonGia = mysqli_fetch_array($dbQuanLyBanLinhKien->query($query_dongia));
					$donGia = $arrdonGia['don_gia'];
					$query_insertInfoBill = "INSERT INTO  ChiTietDonHang(so_hoa_don,ma_san_pham,so_luong, don_gia) VALUES('".$numBillUser."', '".$maSanPham."',".$_COOKIE[$maSanPham].",".$donGia.")";
					$dbQuanLyBanLinhKien->query($query_insertInfoBill);
				}
			}	
		}
