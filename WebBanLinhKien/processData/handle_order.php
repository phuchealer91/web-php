...
if(isset($_COOKIE['donHang'])){
		...
		foreach($arrayMaSanPham as $maSanPham){
				$query_product = "SELECT * FROM HangHoa where ma_san_pham = ".$maSanPham;
...
if(isset($_SESSION['userName']) && isset($_SESSION['pass'])){
	...
	if(isset($_REQUEST['session'])){
		...
		$query_mnd = "select ma_nguoi_dung from TaiKhoan where ten_dang_nhap = '".$tenDangNhap."' and mat_khau = '".$matKhau."'";
		$tbMnd = mysqli_fetch_array($dbQuanLyBanLinhKien->query($query_mnd));
		$mnd = $tbMnd['ma_nguoi_dung'];
		$diaChi = $_REQUEST['diaChi'];
		$query_dat_hang = "UPDATE DatHang SET trang_thai = 1, dia_chi = '".$diaChi."', tri_gia = ".$total." WHERE ma_user = $mnd and trang_thai = 0";
	...
	$query_newUser = "INSERT INTO  KhachHang(ten_khach_hang, dia_chi, dien_thoai, email) 
