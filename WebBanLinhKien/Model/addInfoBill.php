...
{ 
	...
	if(isset($_SESSION['userName']) && isset($_SESSION['pass'])){
		...
		//Lấy mã người dùng
		$query_maNguoiDung = "select ma_nguoi_dung from TaiKhoan where ten_dang_nhap = '".$tenDangNhap."' and mat_khau = '".$matKhau."'";
		...
		if($checkBillUser > 0){
			...
			if($checkInfoBillUser > 0){
				...
				if($checkExist == 0){
					$query_dongia = 'select don_gia from HangHoa where ma_san_pham ='.$ma_sp_mua;
					$arrdonGia = mysqli_fetch_array($dbQuanLyBanLinhKien->query($query_dongia));
					$donGia = $arrdonGia['don_gia'];
					$query_insertInfoBill = "INSERT INTO  ChiTietDonHang(so_hoa_don,ma_san_pham,so_luong, don_gia) VALUES('".$soHoaDon."', '".$ma_sp_mua."',".$so_luong_sp.",".$donGia.")";
			...
			$arrdonGia = mysqli_fetch_array($dbQuanLyBanLinhKien->query($query_dongia));
			$donGia = $arrdonGia['don_gia'];
			$query_insertInfoBill = "INSERT INTO  ChiTietDonHang(so_hoa_don,ma_san_pham,so_luong, don_gia) VALUES('".$numBillUser."', '".$ma_sp_mua."',".$so_luong_sp.",".$donGia.")";
			$dbQuanLyBanLinhKien->query($query_insertInfoBill);
		}
	}
	//xử lý thêm cookie
