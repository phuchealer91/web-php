...
if(!empty($_REQUEST["ma_sp_capnhat"]))
{
	...
	if(isset($_SESSION['userName']) && isset($_SESSION['pass'])){
		...
		if($so_luong_cap_nhat == 0){
			$query_delete = 'delete from ChiTietDonHang where ma_san_pham = "'.$ma_sp_cap_nhat.'" and so_hoa_don = "'.$soHoaDon.'"';
			$dbQuanLyBanLinhKien->query($query_delete);	
			
		}
		else{
			$query_update = 'update ChiTietDonHang set so_luong ='.$so_luong_cap_nhat.' where ma_san_pham = "'.$ma_sp_cap_nhat.'" and so_hoa_don = "'.$soHoaDon.'"';
